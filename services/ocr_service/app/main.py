...
app = FastAPI(title="SARATHI OCR Service", version="0.1.0", lifespan=lifespan)
app.add_middleware(PrincipalMiddleware, settings=AuthSettings())
app.add_middleware(AuditLoggingMiddleware)
app.include_router(router, prefix="/ocr/v1")
...
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    if hasattr(exc, "status_code") and hasattr(exc, "detail"):
        ...
        if isinstance(detail, dict):
            return JSONResponse(status_code=getattr(exc, "status_code", 500), content=detail)
